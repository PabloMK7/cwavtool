...
/*
 * Calculate the initial deltas from a PCM block.
 * initial_deltas -> output
 * num_channels -> channels in the stream
 * inbuf -> input PCM16 samples
...
{
    int i;
    initial_deltas[0] = initial_deltas[1] = 0;

    for (i = inbufcount * num_channels; i -= num_channels;) {
        initial_deltas[0] -= initial_deltas[0] >> 3;
        initial_deltas[0] += abs ((int32_t) inbuf[i] - inbuf[i - num_channels]);

        if (num_channels == 2) {
            initial_deltas[1] -= initial_deltas[1] >> 3;
            initial_deltas[1] += abs ((int32_t) inbuf[i-1] - inbuf[i+1]);
        }
    }
...
int adpcm_encode_block (void *p, IMAADPCMInfo* infos, uint8_t *outbuf, size_t *outbufsize, const int16_t *inbuf, int inbufcount)
{
    ...
    *outbufsize = 0;
    ...
    for (ch = 0; ch < pcnxt->num_channels; ch++) {
        ...
        infos[ch].sample = (uint16_t)init_pcmdata[ch];
        infos[ch].index = init_index[ch];
    }
